/***************************************************************************
 * * NodeImpl.java * ------------------- * date : 16.08.2004 * copyright : (C)
 * 2004-2008 Distributed and * Mobile Systems Group * Lehrstuhl fuer Praktische
 * Informatik * Universitaet Bamberg * http://www.uni-bamberg.de/pi/ * email :
 * sven.kaffille@uni-bamberg.de * karsten.loesing@uni-bamberg.de * * *
 ***************************************************************************/
...
public final class NodeImpl extends Node {
    ...
    NodeImpl(ChordImpl impl, ID nodeID, URL nodeURL,
            NotifyCallback nodeCallback, References references,
            Entries entries) {
    ...
    public final List<Node> notify(Node potentialPredecessor) {
        ...
        try {
            ...
            this.references
                    .addReferenceAsPredecessor(potentialPredecessor);
            return result;
        } finally {
    ...
    public final RefsAndEntries notifyAndCopyEntries(
        ...
        try {
            ...
            // predecessor
            Set<Entry> copiedEntries = this.entries
                    .getEntriesInInterval(this.nodeID,
                            potentialPredecessor.getNodeID());

            return new RefsAndEntries(this
                    .notify(potentialPredecessor), copiedEntries);
        } finally {
            this.notifyLock.unlock();
        }
    }
    ...
    public final void insertEntry(Entry toInsert)
        ...
        if (this.logger.isEnabledFor(DEBUG)) {
            this.logger.debug("Inserting entry with id "
                    + toInsert.getId() + " at node " + this.nodeID);
        }
    ...
    public final void removeEntry(Entry entryToRemove)
        ...
        if (this.logger.isEnabledFor(DEBUG)) {
            this.logger.debug("Removing entry with id "
                    + entryToRemove.getId() + " at node "
                    + this.nodeID);
        }
        ...
        if (this.references.getPredecessor() != null
                && !entryToRemove.getId().isInInterval(
                        this.references.getPredecessor().getNodeID(),
                        this.nodeID)) {
            this.references.getPredecessor().removeEntry(
                    entryToRemove);
            return;
        }
    ...
    public final void removeReplicas(ID sendingNodeID,
        ...
        if (replicasToRemove.size() == 0) {
            ...
            if (debug) {
                this.logger
                        .debug("Removing replicas. Current no. of entries: "
                                + this.entries
             ...
             */
            Set<Entry> allReplicasToRemove = this.entries
                    .getEntriesInInterval(this.nodeID, sendingNodeID);
            if (debug) {
                this.logger.debug("Replicas to remove "
                        + allReplicasToRemove);
    ...
    public final Set<Entry> retrieveEntries(ID id)
        ...
        if ((this.references.getPredecessor() != null)
            ...
            this.logger
                    .fatal("The rare situation has occured at time "
                            ...
                            + this.references.getPredecessor()
                                    .getNodeID());
            return this.references.getPredecessor().retrieveEntries(
                    id);
        }
        // added by INET
    ...
    final public void leavesNetwork(Node predecessor) {
        if (this.logger.isEnabledFor(INFO)) {
            this.logger.info("Leaves network invoked; " + this.nodeID
                    + ". Updating references.");
            this.logger.info("New predecessor "
        ...
        this.references.removeReference(this.references
                .getPredecessor());
        if (this.logger.isEnabledFor(DEBUG)) {
    ...
    public final void broadcast(Broadcast info)
        ...
        if (this.notifyCallback != null) {
            this.notifyCallback.broadcast(info.getSource(), info
